...
const VerifyPage = () => {
  ...
  useEffect(() => {
    fetch(
      'https://wallet-project-m5us.onrender.com/api/user/verify/' + verifyCode,
      {
        method: 'POST'
      }
    )
    ...
    <>
      <CenterDiv>
        ...
        {verify?.token && (
          <div>
            {verify.message}{' '}
            <p>You will be redirected to your cabinet in 5 seconds</p>{' '}
            <ModalLogoutButtonNo onClick={doRedirect}>
              Enter cabinet
            </ModalLogoutButtonNo>
          </div>
        )}
        {verify === 404 && (
          <p>User already has verified or invalid verify code</p>
        )}
      </CenterDiv>
